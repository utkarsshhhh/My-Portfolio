      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-12 items-center">
        <div className={`md:col-span-5 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-8'}`}>
          <div className="relative group">
            ...
            <img
              ...
              alt="Utkarsh Adlak"
              className="relative w-full max-w-sm mx-auto rounded-full glowing-shadow transition-transform duration-500 group-hover:-translate-y-2 group-hover:scale-105"
            />
          </div>
        </div>
        <div className={`md:col-span-7 transition-all duration-1000 delay-300 ${isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-8'}`}>
          ...
          <p className="text-lg text-gray-300 mb-8 leading-relaxed">
            “I believe technology is a tool to solve problems and create change. I'm <b>Utkarsh Adlak</b>, a web developer dedicated to learning and contributing innovative solutions. My focus is on building scalable, modern applications while continuously improving my skills and pushing boundaries.”
          </p>
          <div className="flex flex-wrap items-center gap-6">
            ...
            {skills.map((skill, index) => (
              <div 
                key={skill.name} 
                className={`group flex flex-col items-center gap-2 transition-all duration-500 ease-out hover:scale-110 hover:-translate-y-1 ${isVisible ? 'opacity-100 translate-y-0 rotate-0' : 'opacity-0 translate-y-4 -rotate-6'}`}
                style={{ transitionDelay: `${400 + index * 100}ms` }}
              >
                <i className={`ph-bold ${skill.icon} text-4xl text-[#D8ECF8] transition-all duration-300 group-hover:text-white group-hover:[text-shadow:0_0_10px_rgba(216,236,248,0.8)]`}></i>
